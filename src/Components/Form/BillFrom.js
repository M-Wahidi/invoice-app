...
function BillFrom({ handleAddItem, addFromAddress, openForm, title, getInvoiceBillFormData }) {
  ...
  return (
    <div className='bill-from-container' style={{ paddingTop: "3rem" }}>
      ...
      <div className='street-address-from'>
        <label style={{ color: `${theme ? "#333" : "#fff"}` }} htmlFor='street-address-from'>
          Street Address
        </label>
        <input
          type='text'
      ...
      >
        <div className='city-from' style={{ flex: 1, minWidth: "180px" }}>
          <label style={{ color: `${theme ? "#333" : "#fff"}` }} htmlFor='city-from'>
            City
          </label>
          <input
            type='text'
